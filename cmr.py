...
for row in reader:
  ...
  row[6]=time_to_string(row[6])  

  #row[ 7]=int_to_ip(row[ 7])

  # 3,5,7,8,10,12,13,14,18 


  qos=row[18].split(";")
  cs="0"
  scs="0"	
  for parameter in qos:
	try:
		kpi,value = parameter.split("=",1)
		if (kpi == "CS"):
			cs=value
		if (kpi == "SCS"):
			scs=value
  ...
  print ','.join([row[6],row[4],row[7],row[9],row[11],row[12],row[13],cs,scs,row[16],row[2],row[17]])  
